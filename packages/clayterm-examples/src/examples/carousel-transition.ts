...
const example: ExampleDefinition<CarouselState, Op> = {
  ...
  view(state) {
    ...
    const pushTransitionSlide = (key: string, slideIndex: number, direction: 1 | -1, entering: boolean) => {
      const enterPreset = entering
        ? direction === 1 ? TRANSITION_PRESET.ENTER_FROM_RIGHT : TRANSITION_PRESET.ENTER_FROM_LEFT
        : TRANSITION_PRESET.NONE;
      const exitPreset = !entering
        ? direction === 1 ? TRANSITION_PRESET.EXIT_TO_LEFT : TRANSITION_PRESET.EXIT_TO_RIGHT
        : TRANSITION_PRESET.NONE;

      ops.push(open(`slide-${key}`, {
        layout: {
        ...
        transition: {
          ...
          enter: { preset: enterPreset, trigger: TRANSITION_ENTER_TRIGGER.TRIGGER_ON_FIRST_PARENT_FRAME },
          exit: {
            preset: exitPreset,
      ...
      pushSlideRows(ops, buildSlideRows(slides[slideIndex], metrics.frameInnerWidth, metrics.frameInnerHeight), palette.frameBg);
      ops.push(close());
    };
  ...
  reduce(state, inputEvents) {
    ...
    for (const event of inputEvents) {
      ...
      if (event.type === "mouseup") {
        ...
        if (!next.animating && (event.button === "left" || event.button === "release") && pressed && hovered === pressed) {
          next.previousSlide = next.currentSlide;
